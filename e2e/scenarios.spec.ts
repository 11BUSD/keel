...
test("cohort cascade: throttled unwind walks the loss down the waterfall", async ({
  ...
  await expect(page.getByText("throttled unwind")).toBeVisible({ timeout: 15_000 });
...
test("oracle failure HALTS the fleet; reset restores it", async ({ page }) => {
  ...
  await expect(page.getByText("HALTED — fail-safe default")).toBeVisible({
    timeout: 15_000,
  });
  ...
  await expect(page.getByText("No scenario run yet")).toBeVisible();
});
